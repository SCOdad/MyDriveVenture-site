...
(()=>{
  ...
  async function init(){const result=await client.auth.getSession();session=result.data.session;if(!session){loading.hidden=true;authNeeded.hidden=false;return}try{await refresh();const requestedDriverId=new URLSearchParams(location.search).get('driver');if(requestedDriverId){const target=subjects.find(s=>String(s.driver_id||'')===String(requestedDriverId));if(target){subjectSelect.value=String(target.person_id);renderSubject()}}loading.hidden=true;app.hidden=false;if(location.hash==='#license-card'&&!licenseCard?.hidden)setTimeout(()=>licenseCard.scrollIntoView({behavior:'smooth',block:'start'}),0)}catch(e){loading.innerHTML=`<p>${esc(e.message||e)}</p>`}}
