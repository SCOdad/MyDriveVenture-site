(()=>{
  ...
  let session=null,subjects=[],contactChangesAvailable=false
  ...
  async function refresh(keepPersonId=null){const result=await call('overview');subjects=result.subjects||[];contactChangesAvailable=result.contact_changes_available===true;if(!subjects.length)throw new Error('No editable profiles are available to this account.');render();if(keepPersonId&&subjects.some(s=>String(s.person_id)===String(keepPersonId))){subjectSelect.value=String(keepPersonId);renderSubject()}}
  ...
  async function contactChange(kind){if(!contactChangesAvailable){status('contact-status','Verified contact changes are not available yet.');return}status('contact-status',`Checking ${kind} change options…`);try{await call('request_contact_change',{person_id:selected()?.person_id,endpoint_type:kind.toUpperCase()})}catch(err){status('contact-status',err.message||'Unable to start contact change.','error')}}
