(() => {
  ...
  function operatorBubble(){
    const candidates=[...document.querySelectorAll('div,section,aside,p,span')].filter(el=>{const t=String(el.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();return t.includes('operator view')&&(t.includes('read only')||t.includes('read-only'))}).sort((a,b)=>String(a.textContent||'').length-String(b.textContent||'').length);
    const label=candidates[0];if(!label)return null;
    return label.closest('[class*="operator"],[class*="notice"],[class*="banner"],[class*="pill"],[class*="badge"],.app-card')||label.parentElement;
  }
  function ensureSearch(){
    const switcher=document.getElementById('driver-switcher'),select=document.getElementById('driver-select');if(!switcher||!select||!model?.is_operator)return;switcher.hidden=false;
    let wrap=document.getElementById('operator-driver-search-wrap');if(!wrap){wrap=document.createElement('div');wrap.id='operator-driver-search-wrap';wrap.className='operator-driver-search';wrap.innerHTML='<label for="operator-driver-search"><span>Find driver</span><input id="operator-driver-search" type="search" placeholder="Name, email, phone, or ID" autocomplete="off"></label><div class="operator-repair-row"><button id="operator-recompute-progress" class="button subtle-button button-small" type="button">Recompute progress</button><span id="operator-driver-search-status" class="meta"></span></div>';const style=document.createElement('style');style.textContent='.operator-driver-search{min-width:min(28rem,75vw);margin:.55rem 0}.operator-driver-search label{display:grid;gap:.2rem}.operator-driver-search span{font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em}.operator-driver-search input{width:100%;box-sizing:border-box;padding:.55rem .65rem;border:1px solid #b9b9b9;border-radius:.35rem;background:#fff;color:#111}.operator-repair-row{display:flex;align-items:center;gap:.5rem;margin-top:.3rem}.operator-driver-search .meta{font-size:.72rem}';document.head.appendChild(style);wrap.querySelector('input').addEventListener('input',e=>filterDrivers(e.target.value));wrap.querySelector('#operator-recompute-progress').addEventListener('click',recomputeProgress)}
    const target=operatorBubble()||switcher;if(wrap.parentElement!==target)target.appendChild(wrap)
  }
