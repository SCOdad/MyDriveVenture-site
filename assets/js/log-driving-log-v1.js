(() => {
  ...
  client.functions.invoke=async(slug,options={})=>{
    if(slug==='drive-detail-api'){
      const result=await originalInvoke(slug,options),drive=result?.data?.drive,body=options?.body||{};
      if(!result?.error&&drive?.id&&body.driver_id){const skills=await originalInvoke('drive-skill-ops',{body:{action:'get',driver_id:body.driver_id,drive_id:drive.id}});if(!skills.error&&skills.data?.ok){drive.lesson_ids=skills.data.lesson_ids||[];drive.lessons=skills.data.lessons||[];if(drive.lesson_ids.length)drive.lesson_id=drive.lesson_ids[0]}}
      return result;
    }
    if(slug==='drive-ops'&&['log_drive','edit_drive'].includes(options?.body?.action)){
      ...
      if(!result?.error&&result?.data?.ok&&drive?.id&&driverId){const synced=await originalInvoke('drive-skill-ops',{body:{action:'set',driver_id:driverId,drive_id:drive.id,lesson_ids:requestedLessonIds}});if(!synced.error&&synced.data?.ok){drive.lesson_ids=synced.data.lesson_ids||requestedLessonIds;drive.lesson_id=drive.lesson_ids[0]||null}else{result.data.skill_sync_warning='Drive saved, but skills could not be updated. Please reopen the drive and try again.';setDriveWarning(result.data.skill_sync_warning)}}
      return result;
    }
  ...
  async function loadContext(driverId){
    ...
    if(mode==='ENUMERATED'){
      ...
      if(lessonWrap){lessonWrap.hidden=true;lessonWrap.style.display='none'}if(lessonNotesWrap){lessonNotesWrap.hidden=false;lessonNotesWrap.style.display='grid'}if(lesson){lesson.multiple=false;lesson.value=''}if(lessonNotes)lessonNotes.required=false;
    }else{if(lessonWrap){lessonWrap.hidden=true;lessonWrap.style.display='none'}if(lessonNotesWrap){lessonNotesWrap.hidden=true;lessonNotesWrap.style.display='none'}}
  ...
  form?.addEventListener('submit',async e=>{
    const driveId=form.dataset.editDrive,driverId=app.getDriverId?.();if(!driveId||!driverId||!lesson?.multiple)return;const original=app.detailDrives?.[driveId];if(!original)return;
    ...
    if(!unchanged)return;e.preventDefault();e.stopImmediatePropagation();if(driveStatus){driveStatus.textContent='Saving skills…';driveStatus.className='app-status'}
    const {data,error}=await originalInvoke('drive-skill-ops',{body:{action:'set',driver_id:driverId,drive_id:driveId,lesson_ids:currentIds}});if(error||!data?.ok){if(driveStatus){driveStatus.textContent='Skills could not be updated.';driveStatus.className='app-status error'};return}
    original.lesson_ids=data.lesson_ids||currentIds;original.lesson_id=original.lesson_ids[0]||null;if(driveStatus){driveStatus.textContent='Skills updated.';driveStatus.className='app-status success'}try{await app.refreshDashboard()}catch(_){}
  },true);
