(() => {
  ...
  // Decorate the existing drive APIs with the many-to-many skill assignments while the
  // legacy lesson_id remains the first selected item for compatibility.
  ...
  // Handle a skill-only edit. The legacy drive editor compares scalar lesson_id, so a
  // secondary add/remove needs its own save path when every other drive fact is unchanged.
