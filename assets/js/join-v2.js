(()=>{
  ...
  const endpoint=cfg.functionUrl('public-acquisition-v2'),storageKey='dv:acquisition:v2:flow';
  let flowId='';
  ...
  request({action:'view',flow_id:flowId}).catch(()=>{});
  ...
  form.addEventListener('submit',async e=>{
    ...
    try{
      await request({action:'submit',flow_id:flowId,name,email,website});
    ...
    }finally{button.disabled=false;button.textContent='Email my secure link'}
  });
})();
