(() => {
  ...
  function resetAfterCreate(){clearEditUi();['drive-start','drive-end','drive-destination','drive-notes'].forEach(id=>{const el=field(id);if(el)el.value=''});window.DV_DRIVING_LOG?.setLessonSelection?.([]);window.DV_DRIVING_LOG?.updateNoteCount?.()}
