(() => {
  ...
  document.addEventListener('click',async e=>{if(e.target.id==='drive-edit-cancel'){cancelEdit();return}const trigger=e.target.closest?.('button[data-edit-drive]');if(!trigger)return;const id=trigger.dataset.editDrive,driverId=app.getDriverId?.();if(!id||!driverId)return;const pending=pendingForCurrentDriver();if(pending){setRecoveryPending(true);setStatus('Finish checking the unfinished save before editing another drive.','error');ensureRecoveryButton().scrollIntoView({behavior:'smooth',block:'center'});return}if(edit?.id===id){form.scrollIntoView({behavior:'smooth',block:'start'});return}setStatus('Opening drive…');const{data,error}=await client.functions.invoke('drive-detail-api',{body:{driver_id:driverId,drive_id:id}});if(error||!data?.drive)return setStatus('Drive details could not be opened.','error');app.detailDrives=app.detailDrives||{};app.detailDrives[id]=data.drive;enterEdit(data.drive);setStatus('')});
  ...
  async function recoverPendingMutation(){
    ...
    try{
      ...
      if(action==='ADOPT'){
        ...
        return setStatus(`Drive edit recovered and verified: ${summary(initial.drive)}.`,'success');
      }
      if(action==='CONFLICT'){
        app.detailDrives=app.detailDrives||{};app.detailDrives[pending.drive_id]=initial.drive;clearPending();enterEdit(initial.drive,{scroll:false,preservePriorDraft:false});setSubmitting(false);
      ...
      return setStatus(`Drive edit recovered and verified: ${summary(reread.drive)}.`,'success');
    }catch(error){
  ...
  form.addEventListener('submit',async e=>{
    ...
    if(edit){
      ...
      try{
        ...
        if(!sameChanged(requested,values(),original)){keepRequestedLoaded(requested);setRecoveryPending(true);return setStatus('Drive edit was saved, but the form could not safely adopt the authoritative values. Use “Check unfinished save” before editing again.','error')}
        clearPending();setSubmitting(false);
