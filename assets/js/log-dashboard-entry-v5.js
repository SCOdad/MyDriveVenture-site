(() => {
  ...
  loginForm.addEventListener('submit',async e=>{e.preventDefault();status(loginStatus,'Sending sign-in link…');const email=loginEmail.value.trim();if(!email)return;const experience=document.documentElement.dataset.experience;const redirectPath=experience==='game-v2'?'/log/game-v2/':experience==='game'?'/log/game/':'/log/';const{error}=await client.auth.signInWithOtp({email,options:{emailRedirectTo:`${window.location.origin}${redirectPath}`,shouldCreateUser:false}});if(error){if(isUnregisteredOtpError(error))joinPrompt();else status(loginStatus,'We could not send a sign-in link right now. Please try again.','error')}else status(loginStatus,'Check your email for a secure sign-in link.','success')});
