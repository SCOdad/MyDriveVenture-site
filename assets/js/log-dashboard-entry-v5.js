(() => {
  ...
  loginForm.addEventListener('submit',async e=>{e.preventDefault();status(loginStatus,'Sending sign-in link…');const email=loginEmail.value.trim();if(!email)return;const route=document.documentElement.dataset.dvRoute||'dv03',redirectPath={dv00:'/log/DV00/',dv01:'/log/DV01/',dv02:'/log/DV02/',dv03:'/log/',dv03v2:'/log/DV03/v2/'}[route]||'/log/';const{error}=await client.auth.signInWithOtp({email,options:{emailRedirectTo:`${window.location.origin}${redirectPath}`,shouldCreateUser:false}});if(error){if(isUnregisteredOtpError(error))joinPrompt();else status(loginStatus,'We could not send a sign-in link right now. Please try again.','error')}else status(loginStatus,'Check your email for a secure sign-in link.','success')});
