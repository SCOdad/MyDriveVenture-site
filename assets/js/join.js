(() => {
  ...
  const priorPracticeHas = form.elements.priorPracticeHas;
  const priorPracticeTotal = form.elements.priorPracticeTotal;
  const priorPracticeNight = form.elements.priorPracticeNight;
  const priorPracticeWrap = document.getElementById('prior-practice-wrap');
  ...
  function clearFieldErrors() { form.querySelectorAll('.field-error').forEach((node) => node.remove()); form.querySelectorAll('[aria-invalid="true"]').forEach((field) => field.removeAttribute('aria-invalid')); [guardianMobile, driverMobile,guardianEmail,driverEmail,homeZip,priorPracticeTotal,priorPracticeNight].forEach((field) => field && field.setCustomValidity('')); }
  ...
  function numericFieldValue(field) { const raw = String(field?.value || '').trim(); return raw === '' ? null : Number(raw); }
  function priorPracticePayload(data) {
    const hasPrior = data.get('priorPracticeHas') === 'on';
    if (!hasPrior) return { has_prior_practice: false };
    return { has_prior_practice: true, total_hours: Number(value(data, 'priorPracticeTotal') || 0), night_hours: Number(value(data, 'priorPracticeNight') || 0) };
  }
  function syncConditionalRequirements() {
    ...
    const hasPrior = Boolean(priorPracticeHas && priorPracticeHas.checked);
    if (priorPracticeWrap) priorPracticeWrap.hidden = !hasPrior;
    if (priorPracticeTotal) priorPracticeTotal.required = hasPrior;
    if (priorPracticeNight) priorPracticeNight.required = hasPrior;
    if (!hasPrior) { if (priorPracticeTotal) priorPracticeTotal.value = ''; if (priorPracticeNight) priorPracticeNight.value = ''; }
    if (hasPrior && priorPracticeTotal && priorPracticeNight) {
      const total = numericFieldValue(priorPracticeTotal);
      const night = numericFieldValue(priorPracticeNight);
      priorPracticeTotal.setCustomValidity(total == null || total < 0 ? 'Enter total prior practice hours, or turn off starting practice balance.' : '');
      priorPracticeNight.setCustomValidity(night == null || night < 0 ? 'Enter prior night hours, or use 0 if none are complete yet.' : '');
      if (total != null && night != null && night > total) priorPracticeNight.setCustomValidity('Night hours cannot exceed total prior practice hours.');
    }
  ...
  async function savePriorPracticeCredit(onboardingResult, priorPractice) {
    if (!priorPractice || priorPractice.has_prior_practice !== true) return null;
    const endpoint = window.DV_ENVIRONMENT_CONFIG?.functionUrl?.('public-prior-practice-credit');
    if (!endpoint) throw new Error('Starting practice balance service is not available.');
    const response = await fetch(endpoint, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ submission_id: onboardingResult.submission_id, family_id: onboardingResult.family_id, driver_id: onboardingResult.driver_id, prior_practice: priorPractice }) });
    const body = await response.json().catch(() => ({}));
    if (!response.ok || body.ok !== true) throw new Error(String(body.error || 'Starting practice balance could not be saved.'));
    return body.prior_practice_credit || null;
  }
  async function submissionFingerprint(data, photoFile) { const prior = priorPracticePayload(data); const stable = { guardian: { name: value(data, 'guardianName'), email: value(data, 'guardianEmail'), mobile: value(data, 'guardianMobile'), sms: data.get('guardianSmsOptIn') === 'on' }, driver: { name: value(data, 'driverName'), birth: value(data, 'driverBirthDate'), email: value(data, 'driverEmail'), mobile: value(data, 'driverMobile'), sms: data.get('driverSmsOptIn') === 'on', zip: value(data, 'homeZip'), state:stateFromZip(value(data,'homeZip')), stage: value(data, 'licenseStage'), stageDate: value(data, 'licenseStageStartDate'), avatar: data.get('customAvatarRequested') === 'on' }, vehicle: { name: value(data, 'vehicleName'), class: value(data, 'vehicleClass'), color: value(data, 'vehicleColor') }, priorPractice: prior, photo: photoFile ? { name: photoFile.name, size: photoFile.size, type: photoFile.type, modified: photoFile.lastModified } : null }; return sha256Hex(JSON.stringify(stable)); }
  ...
  [avatarRequested, guardianSmsOptIn, driverSmsOptIn, guardianMobile, driverMobile,guardianEmail,driverEmail,homeZip,priorPracticeHas,priorPracticeTotal,priorPracticeNight].forEach((field) => { if (!field) return; field.addEventListener('change', () => { clearFieldErrors(); syncConditionalRequirements(); }); field.addEventListener('input', () => { clearFieldErrors(); syncConditionalRequirements(); }); });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const photoFile = avatarPhoto && avatarPhoto.files ? avatarPhoto.files[0] : null; const fingerprint = await submissionFingerprint(data, photoFile); const photo = avatarRequested && avatarRequested.checked ? await fileToPayload(photoFile) : null; const guardianName = value(data, 'guardianName'); const driverName = value(data, 'driverName'); const priorPractice = priorPracticePayload(data);
      const payload = { source_response_id: getStableSubmissionId(fingerprint), submission_fingerprint: fingerprint, website: value(data, 'website'), submission_context: window.DVSubmissionContext?.collect('ONBOARDING') ?? { schema_version: 1, form_source: 'ONBOARDING' }, guardian: { given_name: guardianName, family_name: '', display_name: guardianName, email: value(data, 'guardianEmail'), mobile: value(data, 'guardianMobile'), sms_opt_in: data.get('guardianSmsOptIn') === 'on' }, driver: { given_name: driverName, family_name: '', display_name: driverName, birth_date: value(data, 'driverBirthDate'), email: value(data, 'driverEmail'), mobile: value(data, 'driverMobile'), sms_opt_in: data.get('driverSmsOptIn') === 'on', home_zip: value(data, 'homeZip'), home_state: state, license_stage: value(data, 'licenseStage'), level1_license_date: value(data, 'licenseStageStartDate'), custom_avatar_requested: data.get('customAvatarRequested') === 'on' }, avatar_photo: photo, vehicle: { name: value(data, 'vehicleName'), class: value(data, 'vehicleClass'), color: value(data, 'vehicleColor') } };
      ...
      await savePriorPracticeCredit(result, priorPractice);
