(() => {
  ...
  function syncContact(){if(email) email.required=Boolean(followUp&&followUp.checked&&!accessToken)}
  ...
  if(followUp)followUp.addEventListener('change',syncContact); syncContact(); identifyPilot().catch(()=>{});
  ...
  form.addEventListener('submit',async event=>{
    ...
    const data=new FormData(form), wantsFollowUp=data.get('followUpRequested')==='on';
    const payload={website:value(data,'website'),name:value(data,'name'),email:value(data,'email'),relationship:value(data,'relationship'),category:value(data,'category'),area:value(data,'area'),rating:value(data,'rating'),message:value(data,'message'),follow_up_requested:wantsFollowUp,updates_opt_in:wantsFollowUp,project_state:String(window.DV_PROJECT_STATE||'ALPHA'),code_revision:String(window.DV_CODE_REVISION||''),submission_context:window.DVSubmissionContext?.collect('FEEDBACK')??{schema_version:1,form_source:'FEEDBACK'}};
