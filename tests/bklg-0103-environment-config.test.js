...
test('Cloudflare DEV project and branch previews are pinned to DEV', () => { for (const host of ['drive-venture-dev.pages.dev','bklg-0103-dev-environment.drive-venture-dev.pages.dev','abc123.drive-venture-dev.pages.dev']) { const c=load(host); assert.equal(c.name,'dev'); assert.equal(c.projectRef,'safwylxxhywbsfxpmchd'); } });
