...
async function createFixture(page, marker) {
  ...
  const responsePromise=page.waitForResponse(r=>r.url().includes('/functions/v1/drive-ops')&&r.request().postData()?.includes('log_drive'));
...
test('BKLG-0151 road notes persist through edit, authoritative reread, and reload', async ({ page }) => {
  ...
  const editRequest=page.waitForRequest(r=>r.url().includes('/functions/v1/drive-ops')&&r.request().method()==='POST'&&r.request().postData()?.includes('edit_drive'));
  await page.locator('#drive-form button[type=submit]').click();
  ...
  },{driverId:await page.locator('#driver-select').inputValue(),driveId});
